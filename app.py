...
st.markdown("""
    ...
    .stButton>button { background-color: #006400; color: white; font-size: 20px; border-radius: 12px; height: 55px; border: 2px solid #D4AF37; }
    .stButton>button:hover { background-color: #228B22; border-color: white; }
    .tip-box { background-color: #e8f5e9; border-left: 5px solid #006400; padding: 15px; border-radius: 5px; margin-top: 15px; }
    .status-text { font-size: 0.8rem; font-weight: bold; margin-top: -10px; margin-bottom: 10px; display: block; }
...
with col1:
    ...
    # Added on_change logic or just direct display
    N = st.slider(txt['nitrogen'], 0, 140, 50)
    st.caption(get_soil_status(N, 'N', txt)) # Feature 1: Soil Status
    
    P = st.slider(txt['phosphorus'], 5, 145, 50)
    K = st.slider(txt['potassium'], 5, 205, 50)
    
    ph = st.slider(txt['ph'], 0.0, 14.0, 6.5, step=0.1)
    st.caption(get_soil_status(ph, 'pH', txt)) # Feature 1: Soil Status
...
if st.button(txt['predict_btn']):
    ...
    st.download_button(
        ...
        data=report_text,
        file_name="crop_recommendation.txt",
        mime="text/plain"
    )
